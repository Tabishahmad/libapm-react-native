...
import { envelopeHeader, envelopeItemHeader, envelopeItemPayload, envelopeItems, firstArg, getMockSession, getMockUserFeedback, getSyncPromiseRejectOnFirstCall, } from './testutils';
...
describe('Tests ReactNativeClient', () => {
    describe('initializing the client', () => {
        test('client initializes', () => __awaiter(void 0, void 0, void 0, function* () {
            const client = new ReactNativeClient(Object.assign(Object.assign({}, DEFAULT_OPTIONS), { dsn: EXAMPLE_DSN, transport: () => new NativeTransport() }));
            yield expect(client.eventFromMessage('test')).resolves.toBeDefined();
            // @ts-expect-error: Is Mocked
            yield expect(RN.LogBox.ignoreLogs).toBeCalled();
        }));
        test('invalid dsn is thrown', () => {
            try {
                new ReactNativeClient(Object.assign(Object.assign({}, DEFAULT_OPTIONS), { dsn: 'not a dsn', transport: () => new NativeTransport() }));
            }
            catch (e) {
        ...
        test("undefined dsn doesn't crash", () => {
            expect(() => {
                const backend = new ReactNativeClient(Object.assign(Object.assign({}, DEFAULT_OPTIONS), { dsn: undefined, transport: () => new NativeTransport() }));
                return expect(backend.eventFromMessage('test')).resolves.toBeDefined();
            }).not.toThrow();
        });
        test('use custom transport function', () => __awaiter(void 0, void 0, void 0, function* () {
            var _a, _b;
            const mySend = (_request) => Promise.resolve();
            const myFlush = (timeout) => Promise.resolve(Boolean(timeout));
            const myCustomTransportFn = () => ({
                send: mySend,
                flush: myFlush,
            });
            const client = new ReactNativeClient(Object.assign(Object.assign({}, DEFAULT_OPTIONS), { dsn: EXAMPLE_DSN, transport: myCustomTransportFn }));
            expect((_a = client.getTransport()) === null || _a === void 0 ? void 0 : _a.flush).toBe(myFlush);
            expect((_b = client.getTransport()) === null || _b === void 0 ? void 0 : _b.send).toBe(mySend);
        }));
    });
    describe('onReady', () => {
    ...
    describe('nativeCrash', () => {
        test('calls NativeModules crash', () => {
            ...
            const RN = require('react-native');
            const client = new ReactNativeClient(Object.assign(Object.assign({}, DEFAULT_OPTIONS), { dsn: EXAMPLE_DSN, enableNative: true, transport: () => new NativeTransport() }));
            client.nativeCrash();
    ...
    describe('UserFeedback', () => {
        test('sends UserFeedback to native Layer', () => {
            const mockTransportSend = jest.fn(() => Promise.resolve());
            const client = new ReactNativeClient(Object.assign(Object.assign({}, DEFAULT_OPTIONS), { dsn: EXAMPLE_DSN, transport: () => ({
            ...
            expect(mockTransportSend.mock.calls[0][firstArg][envelopeItems][0][envelopeItemHeader].type).toEqual('user_report');
    ...
    describe('attachStacktrace', () => {
        let mockTransportSend;
        let client;
        beforeEach(() => {
            mockTransportSend = jest.fn(() => Promise.resolve());
            client = new ReactNativeClient(Object.assign(Object.assign({}, DEFAULT_OPTIONS), { attachStacktrace: true, stackParser: defaultStackParser, dsn: EXAMPLE_DSN, transport: () => ({
                    send: mockTransportSend,
                    flush: jest.fn(),
                }) }));
        });
        ...
        const getMessageEventFrom = (func) => func.mock.calls[0][firstArg][envelopeItems][0][envelopeItemPayload];
        test('captureMessage contains stack trace in threads', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    describe('envelopeHeader SdkInfo', () => {
        ...
        beforeEach(() => {
            mockTransportSend = jest.fn(() => Promise.resolve());
            client = new ReactNativeClient(Object.assign(Object.assign({}, DEFAULT_OPTIONS), { dsn: EXAMPLE_DSN, transport: () => ({
                    send: mockTransportSend,
                    flush: jest.fn(),
                }) }));
        });
        ...
        const getSdkInfoFrom = (func) => func.mock.calls[0][firstArg][envelopeHeader].sdk;
    ...
    describe('event data enhancement', () => {
        test('event contains sdk default information', () => __awaiter(void 0, void 0, void 0, function* () {
            var _a;
            const mockedSend = jest.fn().mockResolvedValue(undefined);
            const mockedTransport = () => ({
                send: mockedSend,
                flush: jest.fn().mockResolvedValue(true),
            });
            const client = new ReactNativeClient(Object.assign(Object.assign({}, DEFAULT_OPTIONS), { dsn: EXAMPLE_DSN, transport: mockedTransport }));
            client.captureEvent({ message: 'test event' });
            expect(mockedSend).toBeCalled();
            const actualEvent = (mockedSend.mock.calls[0][firstArg][envelopeItems][0][envelopeItemPayload]);
            expect((_a = actualEvent === null || actualEvent === void 0 ? void 0 : actualEvent.sdk) === null || _a === void 0 ? void 0 : _a.packages).toEqual([
                {
    ...
    describe('normalizes events', () => {
        test('handles circular input', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            expect(mockedSend).toBeCalled();
            const actualEvent = (mockedSend.mock.calls[0][firstArg][envelopeItems][0][envelopeItemPayload]);
            expect(actualEvent === null || actualEvent === void 0 ? void 0 : actualEvent.extra).toEqual({
                circular: {
                    extra: '[Circular ~]',
                },
            });
    ...
    describe('clientReports', () => {
        test('does not send client reports if disabled', () => {
            const mockTransportSend = jest.fn((_envelope) => Promise.resolve());
            const client = new ReactNativeClient(Object.assign(Object.assign({}, DEFAULT_OPTIONS), { dsn: EXAMPLE_DSN, transport: () => ({
                    send: mockTransportSend,
                    flush: jest.fn(),
        ...
        test('send client reports on event envelope', () => {
            ...
            expect(mockTransportSend.mock.calls[0][firstArg][envelopeItems][1][envelopeItemPayload]).toEqual(expect.objectContaining({
                discarded_events: [
                    {
                        reason: 'before_send',
                        category: 'error',
            ...
            expect(client._outcomesBuffer).toEqual([]);
        });
        ...
        test('keeps outcomes in case envelope fails to send', () => {
            const mockTransportSend = jest.fn((_envelope) => rejectedSyncPromise(new SentryError('Test')));
            const client = new ReactNativeClient(Object.assign(Object.assign({}, DEFAULT_OPTIONS), { dsn: EXAMPLE_DSN, transport: () => ({
                    send: mockTransportSend,
                    flush: jest.fn(),
                }), sendClientReports: true }));
        ...
        test('sends buffered client reports on second try', () => {
            const mockTransportSend = getSyncPromiseRejectOnFirstCall(new SentryError('Test'));
            const client = new ReactNativeClient(Object.assign(Object.assign({}, DEFAULT_OPTIONS), { dsn: EXAMPLE_DSN, transport: () => ({
                    send: mockTransportSend,
                    flush: jest.fn(),
                }), sendClientReports: true }));
        ...
        function expectOnlyMessageEventInEnvelope(transportSend) {
            expect(transportSend).toBeCalledTimes(1);
            expect(transportSend.mock.calls[0][firstArg][envelopeItems]).toHaveLength(1);
            expect(transportSend.mock.calls[0][firstArg][envelopeItems][0][envelopeItemHeader]).toEqual(expect.objectContaining({ type: 'event' }));
        }
        function mockDroppedEvent(client) {
...
function mockedOptions(options) {
    return Object.assign({ integrations: [], stackParser: jest.fn().mockReturnValue([]), transport: () => ({
            send: jest.fn(),
            flush: jest.fn(),
        }) }, options);
}
