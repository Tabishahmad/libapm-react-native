...
describe('TouchEventBoundary._onTouchStart', () => {
    ...
    it('register itself as integration', () => {
        ...
        core.getCurrentHub.mockReturnValue({
    ...
    it('sentry-label is preferred over labelName and displayName', () => {
        const { defaultProps } = TouchEventBoundary;
        const boundary = new TouchEventBoundary(Object.assign(Object.assign({}, defaultProps), { labelName: 'custom-sentry-label-name' }));
        const event = {
            _targetInst: {
                elementType: {
                    displayName: 'View',
    ...
    it('ignoreNames', () => {
        const { defaultProps } = TouchEventBoundary;
        const boundary = new TouchEventBoundary(Object.assign(Object.assign({}, defaultProps), { labelName: 'custom-sentry-label-name', ignoreNames: ['View', 'Ignore', /^Connect\(/, new RegExp('^Happy\\(')] }));
        const event = {
            _targetInst: {
                elementType: {
                    name: 'View',
    ...
    it('maxComponentTreeSize', () => {
        const { defaultProps } = TouchEventBoundary;
        const boundary = new TouchEventBoundary(Object.assign(Object.assign({}, defaultProps), { maxComponentTreeSize: 2, labelName: 'custom-sentry-label-name' }));
        const event = {
            _targetInst: {
                elementType: {
                    name: 'View',
